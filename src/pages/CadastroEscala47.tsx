...
import { ArrowRight, ArrowLeft, Loader2, Eye, EyeOff, ChevronDown, Lock, Check, CreditCard, ShieldCheck, PartyPopper, Sparkles, Zap } from "lucide-react";
...
const PRODUCTION_URL = "https://argosx.com.br/escala-47";
const PROMO_FIRST_PRICE = "47,90";
const PROMO_FULL_PRICE = "197,90";

export default function CadastroEscala47() {
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      // Custom event: lead landed on promo page
      w.fbq("trackCustom", "EscalaPromoLead", { content_name: "escala_promo_pageview" });
      setPixelReady(true);
      return;
    }
  ...
  const [form, setForm] = useState({
    name: "", phone: "", email: "", companyName: "", cpfCnpj: "", password: "", confirmPassword: "",
  });
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const eventId = `escala47_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
      ...
      const res = await fetch(
        `https://${projectId}.supabase.co/functions/v1/asaas-checkout-escala47`,
        {
          method: "POST",
      ...
      // Fire Meta pixel — Custom EscalaPromoSignup + EscalaPromoPurchase
      const w = window as any;
      if (w.fbq) {
        ...
        w.fbq("trackCustom", "EscalaPromoSignup", {
          content_name: "Argos X - Escala Promo R$47,90",
          currency: "BRL",
          value: 47.90,
        }, { eventID: eventId });

        if (data.purchaseEventId) {
          w.fbq("trackCustom", "EscalaPromoPurchase", {
            value: data.planValue || 47.90,
            currency: "BRL",
            content_name: "Argos X - Escala Promo R$47,90",
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center max-w-md">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-3">
              Bem-vindo ao Plano Escala! 🎉
            </h1>
            <p className="text-gray-600 mb-2">
              Seu pagamento de <strong>R$ {PROMO_FIRST_PRICE}</strong> foi confirmado. Aproveite todo o poder do plano Escala por 30 dias.
            </p>
            <p className="text-sm text-gray-500 mb-8">
              A partir do 31º dia, o valor passa para <strong>R$ {PROMO_FULL_PRICE}/mês</strong>.
            </p>
            <Button onClick={() => navigate("/auth?returnTo=/dashboard")}
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 md:py-12">
        ...
        >
          <div className="bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl px-5 py-3 flex items-center justify-center gap-2 text-sm font-semibold shadow-md">
            <Sparkles className="w-4 h-4 shrink-0" />
            <span>Oferta exclusiva: 1º mês por R$ {PROMO_FIRST_PRICE} no plano Escala</span>
          </div>
        </motion.div>
        ...
        >
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900 leading-tight">
              Escala por <span className="text-[#1a1a6e]">R$ {PROMO_FIRST_PRICE}</span>
              <br />
              <span className="text-base font-medium text-gray-500">no primeiro mês</span>
            </h1>
            <p className="text-sm text-gray-500 mt-3">
              Tudo do plano Escala. Pague R$ {PROMO_FIRST_PRICE} hoje, R$ {PROMO_FULL_PRICE}/mês a partir do 31º dia.
            </p>
              ...
              <AnimatePresence mode="wait">
                {step === 1 && (
                  ...
                  >
                    ...
                    {/* Plano fixo */}
                    <div className="rounded-lg border-2 border-[#1a1a6e] bg-[#1a1a6e]/5 p-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <Zap className="w-5 h-5 text-[#1a1a6e]" />
                          <span className="font-semibold text-[#1a1a6e]">Plano Escala</span>
                        </div>
                        <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-800">PROMO</span>
                      </div>
                      <div className="flex items-baseline gap-2">
                        <span className="text-3xl font-bold text-gray-900">R$ {PROMO_FIRST_PRICE}</span>
                        <span className="text-sm text-gray-500">no 1º mês</span>
                      </div>
                      <p className="text-xs text-gray-500 mt-1">
                        Depois R$ {PROMO_FULL_PRICE}/mês. Cancele quando quiser.
                      </p>
                    </div>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <label htmlFor="terms" className="text-xs text-gray-500 leading-relaxed cursor-pointer">
                        Eu entendo que o valor de R$ {PROMO_FIRST_PRICE} é cobrado hoje e que, a partir do 31º dia, serei cobrado automaticamente R$ {PROMO_FULL_PRICE}/mês. Posso cancelar a qualquer momento.
                        Mudanças de plano só serão liberadas após o 1º mês promocional.
                        Li e aceito os{" "}
                    ...
                    <p className="text-center text-xs text-gray-500 flex items-center justify-center gap-1.5">
                      ...
                      Seus dados estão seguros. Pagamento processado pelo Asaas.
                    </p>
                  </motion.div>
                )}
                ...
                {step === 2 && (
                  ...
                  >
                    ...
                    <div className="bg-gradient-to-br from-[#1a1a6e]/5 to-amber-50 rounded-lg p-4 mb-2 border border-[#1a1a6e]/10">
                      <p className="text-sm text-gray-700">
                        Plano <strong>Escala</strong> — <strong className="text-[#1a1a6e]">R$ {PROMO_FIRST_PRICE} hoje</strong>
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
                        A partir do 31º dia: R$ {PROMO_FULL_PRICE}/mês. Cancele quando quiser.
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input id="cardNumber" value={cardForm.number}
                          onChange={(e) => setCardForm(p => ({ ...p, number: applyCardNumberMask(e.target.value) }))}
                          placeholder="0000 0000 0000 0000" maxLength={19} className="pl-10" autoComplete="cc-number" />
                        {detectedBrand && (
                    ...
                    <Button type="button" onClick={handleSubmit} disabled={loading}
                      ...
                      {loading ? (
                        ...
                        <>
                          ...
                          Pagar R$ {PROMO_FIRST_PRICE} e ativar Escala
                        </>
                      )}
                    </Button>
          ...
          <div className="text-center mt-6">
            <p className="text-sm text-gray-500">
              ⚡ R$ {PROMO_FIRST_PRICE} no 1º mês · depois R$ {PROMO_FULL_PRICE}/mês
            </p>
          </div>
        </motion.div>
      </main>
