...
export function AgentDetailDialog({ agent, open, onOpenChange, initialTab }: AgentDetailDialogProps) {
  ...
  useEffect(() => {
    if (agent) {
      ...
      setFormData({
        ...
        max_unproductive_messages: (agent as any).max_unproductive_messages ?? 20,
        media_handoff_enabled: (agent as any).media_handoff_enabled ?? false,
  ...
  const handleSave = async () => {
    ...
    const payload: any = {
      ...
      max_unproductive_messages: formData.max_unproductive_messages,
      media_handoff_enabled: formData.media_handoff_enabled,
